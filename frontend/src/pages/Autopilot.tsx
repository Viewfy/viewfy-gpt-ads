...
export function Autopilot({ run, onContinue }: { run: Run; onContinue: () => void }) {
  ...
  const deferred = Boolean(run.campaign.submission_deferred) && !launched
  ...
  return (
    <div className="ui-page campaign-roadmap-page ui-page--actions">
      ...
      <CampaignRoadmap deferred={deferred} />
      <ActionBar title={outcome.title} description={deferred ? 'Your draft is saved. Come back whenever you’re ready.' : outcome.description}>
        {launched

...
export function MindMap({ run, nodes, onChange, onConfirm, confirming }: {
  ...
  return <div className="glass-map">
    ...
    <aside className="map-inspector glass-panel" aria-label="Map details" aria-live="polite">
      ...
      <div className="inspector-launch" style={{ '--action-bar-lift': `${actionBarBottom}px` } as CSSProperties}><h3>Ready for your next ad?</h3><p>Ads stay untouched until you confirm.</p><button type="button" className="map-primary-button" disabled={preparing} onClick={() => onConfirm(competitors.map(c => c.id))}>{preparing ? 'Creating ads…' : 'Create Ads'}<Icon name="arrow" size={19} /></button></div>
    </aside>
